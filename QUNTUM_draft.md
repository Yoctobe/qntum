...
The name expands to *QUAntified Network of Temporal Unfolding Magnitudes*, and despite the contraction it has no relation to quantum mechanics; the objects are classical throughout. Each variable carries a scalar *magnitude* that evolves in discrete time. Magnitudes influence each other through a sparse relationship store: pairwise entries ("$j$ influences $i$ with weight $w$ at lag $\ell$"), and optionally higher-order entries in which two or three sources jointly influence a target through a product term or a user-supplied formula. Each variable is wrapped in an *event* with a declared life cycle — before its start time it contributes nothing; during a formation window its participation ramps linearly; it then holds stable and finally decays exponentially — so that regime entries and exits are modelled explicitly rather than smuggled into coefficients. Data enters the model as robustly standardized increments and leaves it as levels, through an exact inverse transform. Stability of iterated forecasts is guaranteed by construction: the global influence gain is shrunk until the spectral radius of the linearized transition matrix falls below a declared cap.
